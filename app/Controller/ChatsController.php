...
{
    public $name = 'Chats';
    public $components = array('Twilio', 'Nexmomessage', 'Slooce', 'Plivo', 'Bandwidth', 'Signalwire', 'Ytel', 'Telnyx');
    public $uses = array('User', 'UserNumber', 'Contact', 'Log', 'Group', 'GroupSmsBlast', 'ContactGroup', 'Smstemplate', 'ContactNote');
    ...
    function searchcontact()
    {
        ...
        if ($_POST['search'] != '') {
            $contactArr = $this->Contact->find('all', array(
                'conditions' => 
                        array(
                            ...
                            'OR' => array(
                                'MATCH(name) AGAINST("'.$_POST['search'].'")' , 'Contact.phone_number' => $_POST['search']
                            )
                        )
                    )
                );
            ...
            $contactArr = $this->Contact->find('all', array('conditions' => array('Contact.user_id' => $user_id, 'Contact.un_subscribers' => 0), 'fields' => array('Contact.id', 'Contact.favorite', 'Contact.color', 'Contact.phone_number', 'Contact.name', 'Contact.email', 'Contact.lastmsg', 'Contact.created'), 'order' => array('Contact.lastmsg DESC'), 'limit' => 50));
            //$groups = $this->Group->find('all',array('conditions'=>array('Group.user_id'=>$user_id,'Group.active'=>1)));
        }
        //$response =  array_merge($contactArr, $groups);
        //$response = $contactArr;
        $this->set('contacts', $contactArr);
    ...
    function chat($type = null, $contactid = null)
    {
        ...
        if ($user_id > 0) {
            ...
            $logs = $this->Log->find('all', array('conditions' => array('Log.user_id' => $user_id, 'Log.contact_id' => $contactid, 'Log.inbox_type' => 1, 'Log.is_deleted' => 0), 'order' => array('Log.created DESC'), 'limit' => 50));
                        ...
                        if ($userDetails['User']['sms_balance'] > 0) {
                            ...
                            if (API_TYPE == 0) {
                                ...
                                if ($this->Session->check('Subaccount')) {
                                   $log_arr['Log']['name'] = $this->Session->read('Subaccount.first_name').' '.$this->Session->read('Subaccount.last_name');
                                }else{
                                   $log_arr['Log']['name'] = $this->Session->read('User.first_name').' '.$this->Session->read('User.last_name');
                                }
								...
								$response = $this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$contact['Contact']['phone_number'],$from,$body);
								$message_id = '';
								$status = '';
								$errortext = '';
								$credits = 0;
								...
								$response = $this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$contact['Contact']['phone_number'],$from,$body);
								$message_id = '';
								$status = '';
								$errortext = '';
								$credits = 0;
								...
								$response = $this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$contact['Contact']['phone_number'],$from,$body);
								$message_id = '';
								$status = '';
								$errortext = '';
								$credits = 0;
								...
								$response = $this->Telnyx->sendsms(TELNYX_KEY,$contact['Contact']['phone_number'],$from,$body);
								$message_id = '';
								$status = '';
								$errortext = '';
								$credits = 0;
            ...
            if ($userDetails['User']['sms_balance'] > 0) {
                ...
                if (API_TYPE == 0) {
                    ...
                    if (isset($response->ResponseXml->Message->Sid)) {
                        ...
                        $length = mb_strlen($logs['Log']['text_message'],"UTF-8");
			            $gsm = $this->is_gsm($logs['Log']['text_message']);
                        if(!$gsm){//if (strlen($logs['Log']['text_message']) != strlen(utf8_decode($logs['Log']['text_message']))) {
                            $credits = ceil($length / 70);
                        } else {
                            $credits = ceil($length / 160);
                        }
					...
					$response = $this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$logs['Log']['phone_number'],$from,$logs['Log']['text_message']);
					$message_id = '';
					$status = '';
					$errortext = '';
					$credits = 0;
    				...
    				$response = $this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$logs['Log']['phone_number'],$from,$logs['Log']['text_message']);
    				$message_id = '';
    				$status = '';
    				$errortext = '';
    				$credits = 0;
					...
					$response = $this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$logs['Log']['phone_number'],$from,$logs['Log']['text_message']);
					$message_id = '';
					$status = '';
					$errortext = '';
					$credits = 0;
					...
					$response = $this->Telnyx->sendsms(TELNYX_KEY,$logs['Log']['phone_number'],$from,$logs['Log']['text_message']);
					$message_id = '';
					$status = '';
					$errortext = '';
					$credits = 0;
    ...
    function notes($contactid = null)
    {
        $user_id = $this->Session->read('User.id');
        $this->layout = 'popup';
        $contactNotes = $this->ContactNote->find('all', array('conditions' => array('ContactNote.user_id' => $user_id, 'ContactNote.contact_id' => $contactid), 'order' => array('ContactNote.created DESC')));
        $this->set('contactNotes', $contactNotes);
        $this->set('contactID', $contactid);
    }
    
    function addnote($contactid = null)
    {
        $this->layout = 'popup';
        $user_id = $this->Session->read('User.id');
        ...
        if (!empty($this->request->data)) {
            if ($this->Session->check('Subaccount')) {
               $this->request->data['ContactNote']['name'] = $this->Session->read('Subaccount.first_name').' '.$this->Session->read('Subaccount.last_name');
            }else{
               $this->request->data['ContactNote']['name'] = $this->Session->read('User.first_name').' '.$this->Session->read('User.last_name');
            }
            $this->request->data['ContactNote']['user_id'] = $user_id;
            $this->request->data['ContactNote']['contact_id'] = $contactid;
            $this->request->data['ContactNote']['created'] = date('Y-m-d H:i:s');
            $this->ContactNote->save($this->request->data);
            $this->Session->setFlash('Note has been added.');
            $this->redirect(array('controller' => 'chats', 'action' => 'index'));
        }
    }
    ...
    function editnote($id)
    {
        ...
        if (!empty($this->request->data)) {
            $this->request->data['ContactNote']['id'] = $id;
            if ($this->Session->check('Subaccount')) {
               $this->request->data['ContactNote']['name'] = $this->Session->read('Subaccount.first_name').' '.$this->Session->read('Subaccount.last_name');
            }else{
               $this->request->data['ContactNote']['name'] = $this->Session->read('User.first_name').' '.$this->Session->read('User.last_name');
            ...
            $this->request->data['ContactNote']['created'] = date('Y-m-d H:i:s');
            $this->ContactNote->save($this->request->data);
            $this->Session->setFlash('Note has been updated.');
            $this->redirect(array('controller' => 'chats', 'action' => 'index'));
        }else{
            $contactNotes = $this->ContactNote->find('first', array('conditions' => array('ContactNote.id' => $id)));
            $this->set('contactNotes', $contactNotes);
        }
    ...
    {
        $this->autoRender = false;
        $this->ContactNote->delete($id);
        $this->Session->setFlash('Note has been deleted.');
        $this->redirect(array('controller' => 'chats', 'action' => 'index'));
