...
class BandwidthsController extends AppController{
	var $uses = 'Users';
    var $components = array('Bandwidth');
	...
	function searchbynumber(){
		...
		if(!empty($this->request->data)){
			...
			if($this->request->data['type']=='local'){
				if(!empty($this->request->data['state'])){
					$fields['state'] = $this->request->data['state'];
				}
				if(!empty($this->request->data['zip'])){
					$fields['zip'] = $this->request->data['zip'];
				}
				if(!empty($this->request->data['areaCode'])){
					$fields['areaCode'] = $this->request->data['areaCode'];
				}
				$response = $this->Bandwidth->listNumbers(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$type,$fields);
	...
	function assignthisnumber(){
		...
		if(!empty($this->request->data['number'])){
			...
			if(BANDWIDTH_APPID=='' && trim($someone['User']['sid']) == ''){
					...
					foreach($response as $response_arr){
						if($response_arr->name ==SITENAME){
							...
							if($this->Config->save($config)){
								$application_id = $response_arr->id;
							}
						}
					}
				}
			...
			if (trim($someone['User']['app_id']) != '') {
                $application_id = trim($someone['User']['app_id']);
            }else {            
                $application_id =  BANDWIDTH_APPID;
            }
			$response = $this->Bandwidth->assignthisnumber(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$this->request->data['number'],$application_id);
			if(empty($response)){
				Controller::loadModel('User');
				$user_id = $this->Session->read('User.id');
				$someone = $this->User->find('first', array('conditions' => array('User.id' => $user_id)));
				if ($someone['User']['assigned_number'] == 0) {
					...
					$this->User->saveField('assigned_number', $this->request->data['number']);
					$this->User->saveField('voice', 1);
					$this->User->saveField('sms', 1);										
					if($this->request->data['type'] =='local'){												
					    $this->User->saveField('mms', 1);
					}					
	...
	function sendsms($id = null){
        ...
        if ($userDetails['User']['sms_balance'] > 0) {
            $to = ($this->request->data['Bandwidth']['phone_number']) ? $this->request->data['Bandwidth']['phone_number'] : $this->request->data['Bandwidth']['phone'];
			...
			$body = $this->request->data['Bandwidth']['message'];  
	...
	function sms(){
        ...
        $_REQUEST['Body'] = trim($_REQUEST['text']);
        $_REQUEST['SmsStatus'] = trim($_REQUEST['state']);
		$_REQUEST['media'] = '';
        $phone = $_REQUEST['From'];
                ...
                if (!empty($groupbroadcast)) {
                    ...
                    if (!empty($contactlist)) {
                        ...
                        if ($sms_balance < ($totalSubscriber * $contactcredits)) {
                            ...
                            if ($SID == ''){
                                $this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$_REQUEST['From'],$_REQUEST['To'],$message);
                            }else{
                                $this->Bandwidth->sendsms($SID,$AUTHTOKEN,$BANDWIDTHUSERID,$_REQUEST['From'],$_REQUEST['To'],$message);
                            }
                        ...
                        foreach ($contactlist as $contactlists) {
                            ...
                            $bodymsg = $checkmsgpart[1];
                            if ($SID == ''){
                                $response = $this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$tonumber,$fromnumber,$bodymsg);
                            }else{
                                $response = $this->Bandwidth->sendsms($SID,$AUTHTOKEN,$BANDWIDTHUSERID,$tonumber,$fromnumber,$bodymsg);
                            }
            ...
            if ($bday == 1) {
                ...
                if (!empty($contact)) {
                    ...
                    if ($someoneuser['User']['sms_balance'] > 0) {
                        ...
                        if ($SID == ''){
                            $this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$_REQUEST['From'],$_REQUEST['To'],$message,1);
                        }else{
                            $this->Bandwidth->sendsms($SID,$AUTHTOKEN,$BANDWIDTHUSERID,$_REQUEST['From'],$_REQUEST['To'],$message,1);
                        }
                        $this->smsmail($someone['User']['id']);
                        exit;
                    }
		...
		$multicontestkeywords = $this->MultiContest->find('first',array('conditions'=>array('MultiContest.user_id'=>$someone['User']['id'],'upper(MultiContest.keyword)'=>strtoupper($_REQUEST['Body']))));
		if(!empty($multicontestkeywords)){
			...
			if($multicontestkeywords['MultiContest']['group_id'] > 0){
				...
				if(empty($contact)){
					...
					if(empty($contactgroupids)){
						...
						$contactgroups_arr['ContactGroup']['group_subscribers'] = $_REQUEST['Body'];
			...
			if(!empty($multicontestsubscribers)){
				...
				//}
    				...
    				$multicontest_subscribers['MultiContestSubscriber']['answer'] = $_REQUEST['Body'].''.$description_answer;
    				...
    				$multicontest_subscribers_archive['MultiContestSubscriberArchive']['answer'] = $_REQUEST['Body'].''.$description_answer;
    								...
    								if (strpos($response_phone, ',') !== false) {
    									...
    									if(!empty($response_phone_arr)){
    										foreach($response_phone_arr as $response_phone_arrs){
    											try {
    												if ($SID == ''){
    													$this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$response_phone_arrs,$_REQUEST['To'],$response_text_msg,1);
    												}else{
    													$this->Bandwidth->sendsms($SID,$AUTHTOKEN,$BANDWIDTHUSERID,$response_phone_arrs,$_REQUEST['To'],$response_text_msg,1);
    												}
    									...
    									try {
    										if ($SID == ''){
    											$this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$response_phone,$_REQUEST['To'],$response_text_msg,1);
    										}else{
    											$this->Bandwidth->sendsms($SID,$AUTHTOKEN,$BANDWIDTHUSERID,$response_phone,$_REQUEST['To'],$response_text_msg,1);
    										}
    										$length = mb_strlen($response_text_msg,"UTF-8");
    										$gsm = $this->is_gsm($response_text_msg);
    										if(!$gsm){
        ...
        if (strtoupper(trim($confirmkeyword)) == strtoupper($_REQUEST['Body'])) {
            ...
            if (!empty($appointment)) {
                ...
                if ($SID == ''){
                    $this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$_REQUEST['From'],$_REQUEST['To'],$confirmmessage,1);
                }else{
                    $this->Bandwidth->sendsms($SID,$AUTHTOKEN,$BANDWIDTHUSERID,$_REQUEST['From'],$_REQUEST['To'],$confirmmessage,1);
                }
                $this->User->id = $someone['User']['id'];
                if ($this->User->id != '') {
                    $this->User->saveField('sms_balance', ($someone['User']['sms_balance'] - CHARGEINCOMINGSMS - 1));
            ...
            if (!empty($appointment)) {
                ...
                if ($SID == ''){
                    $this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$_REQUEST['From'],$_REQUEST['To'],$cancelmessage,1);
                }else{
                    $this->Bandwidth->sendsms($SID,$AUTHTOKEN,$BANDWIDTHUSERID,$_REQUEST['From'],$_REQUEST['To'],$cancelmessage,1);
                }
                $this->User->id = $someone['User']['id'];
                if ($this->User->id != '') {
                    $this->User->saveField('sms_balance', ($someone['User']['sms_balance'] - CHARGEINCOMINGSMS - 1));
            ...
            if (!empty($appointment)) {
                ...
                if ($SID == ''){
                    $this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$_REQUEST['From'],$_REQUEST['To'],$reschedulemessage,1);
                }else{
                    $this->Bandwidth->sendsms($SID,$AUTHTOKEN,$BANDWIDTHUSERID,$_REQUEST['From'],$_REQUEST['To'],$reschedulemessage,1);
                }
                $this->User->id = $someone['User']['id'];
                if ($this->User->id != '') {
                    $this->User->saveField('sms_balance', ($someone['User']['sms_balance'] - CHARGEINCOMINGSMS - 1));
        ...
        if (!empty($smsloyalty_status)) {
            if ($someone['User']['sms_balance'] > 0) {
                ...
                if (!empty($contactgroupid)) {
                    ...
                    if (!empty($loyaltyuser)) {
                        ...
                        if ($SID == ''){
                            $this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$_REQUEST['From'],$_REQUEST['To'],$statusmsg,1);
                        }else{
                            $this->Bandwidth->sendsms($SID,$AUTHTOKEN,$BANDWIDTHUSERID,$_REQUEST['From'],$_REQUEST['To'],$statusmsg,1);
                        }
                                ...
                                if ($smsloyalty_arr['Smsloyalty']['reachgoal'] == 1) {
                                    ...
                                    if ($this->SmsloyaltyUser->save($loyalty_user)) {
                                        if ($smsloyalty_arr['Smsloyalty']['type'] == 1) {
                                            ...
                                            $sms = $msg . ' ' . $redeem;
                                            if ($SID == ''){
                                                $this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$_REQUEST['From'],$_REQUEST['To'],$sms,1);
                                            }else{
                                                $this->Bandwidth->sendsms($SID,$AUTHTOKEN,$BANDWIDTHUSERID,$_REQUEST['From'],$_REQUEST['To'],$sms,1);
                                            }
                                            //$this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$_REQUEST['From'],$_REQUEST['To'],$sms);
                                            $this->smsmail($someone['User']['id']);
                                            //*********** Save to activity timeline
                                            $timeline['ActivityTimeline']['user_id'] = $someone['User']['id'];
                                            $timeline['ActivityTimeline']['contact_id'] = $contactgroupid['ContactGroup']['contact_id'];
											...
											if ($SID == ''){
                                                $this->Bandwidth->sendmms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$_REQUEST['From'],$_REQUEST['To'],$sms,$media,1);
                                            }else{
                                                $this->Bandwidth->sendmms($SID,$AUTHTOKEN,$BANDWIDTHUSERID,$_REQUEST['From'],$_REQUEST['To'],$sms,$media,1);
                                            }
											//$this->Bandwidth->sendmms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$_REQUEST['From'],$_REQUEST['To'],$sms,$media);
                                            $this->smsmail($someone['User']['id']);
                                            //*********** Save to activity timeline
                                            $timeline['ActivityTimeline']['user_id'] = $someone['User']['id'];
                                            $timeline['ActivityTimeline']['contact_id'] = $contactgroupid['ContactGroup']['contact_id'];
                                    ...
                                    if ($SID == ''){
                                        $this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$_REQUEST['From'],$_REQUEST['To'],$msg,1);
                                    }else{
                                        $this->Bandwidth->sendsms($SID,$AUTHTOKEN,$BANDWIDTHUSERID,$_REQUEST['From'],$_REQUEST['To'],$msg,1);
                                    }
                                    //$this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$_REQUEST['From'],$_REQUEST['To'],$msg);
                                    $this->smsmail($someone['User']['id']);
                                    //*********** Save to activity timeline
                                    $timeline['ActivityTimeline']['user_id'] = $someone['User']['id'];
                                    $timeline['ActivityTimeline']['contact_id'] = $contactgroupid['ContactGroup']['contact_id'];
        ...
        } else if (strtoupper(trim($_REQUEST['Body'])) == 'START' || strtoupper(trim($_REQUEST['Body'])) == 'UNSTOP') {
            app::import('Model', 'User');
            $this->User = new User();
            $user_id = $someone['User']['id'];
            $sms_balance = $someone['User']['sms_balance'];
        ...
        }else if (strtoupper(trim($_REQUEST['Body'])) == 'HELP') {
            app::import('Model', 'User');
            $this->User = new User();
            $user_id = $someone['User']['id'];
            $sms_balance = $someone['User']['sms_balance'];
            ...
            if (!empty($contacts)) {
                ...
                $this->request->data['User']['sms_balance'] = $sms_balance - CHARGEINCOMINGSMS - 1;
                $this->User->save($this->request->data);
                $message = 'You have successfully been unsubscribed. Reply UNSTOP to get added back to our list. Msg&Data Rates May Apply.';
                if ($SID == ''){
                    $this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$_REQUEST['From'],$_REQUEST['To'],$message,1);
                }else{
                    $this->Bandwidth->sendsms($SID,$AUTHTOKEN,$BANDWIDTHUSERID,$_REQUEST['From'],$_REQUEST['To'],$message,1);
        ...
        //****DOUBLE OPT-IN CHANGES****//    
            ...
            if ($sms_type == 1) {
                if ($SID == ''){
                    $this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$phone,$sender_number,$message,1);
                }else{
                    $this->Bandwidth->sendsms($SID,$AUTHTOKEN,$BANDWIDTHUSERID,$phone,$sender_number,$message,1);
                }
				//$this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$phone,$sender_number,$message);
				sleep(2);
                $this->Immediatelyresponder($user_id, $group_id, $phone, $sender_number);
            } else if ($sms_type == 2) {
                $message_arr = explode(',', $image_url);
                if ($SID == ''){
                    $this->Bandwidth->sendmms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$phone,$sender_number,$message,$message_arr,1);
                }else{
                    $this->Bandwidth->sendmms($SID,$AUTHTOKEN,$BANDWIDTHUSERID,$phone,$sender_number,$message,$message_arr,1);
                }
				//$this->Bandwidth->sendmms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$phone,$sender_number,$message,$message_arr);
				sleep(2);
                $this->Immediatelyresponder($user_id, $group_id, $phone, $sender_number);
            }
            ...
            if ($contact_doubleoptin['ContactGroup']['subscribed_by_sms']==1){
                if ($someone['User']['capture_email_name'] == 0) {
                    ...
                    if ($SID == ''){
                        $this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$phone,$sender_number,$capture_email_name,1);
                    }else{
                        $this->Bandwidth->sendsms($SID,$AUTHTOKEN,$BANDWIDTHUSERID,$phone,$sender_number,$capture_email_name,1);
                    }
					//$this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$phone,$sender_number,$capture_email_name);
                    sleep(2);
                ...
                if ($group['Group']['bithday_enable'] == 1) {
                    if ($someone['User']['birthday_wishes'] == 0) {
                        ...
                        if ($SID == ''){
                            $this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$phone,$sender_number,$birthday_wishes,1);
                        }else{
                            $this->Bandwidth->sendsms($SID,$AUTHTOKEN,$BANDWIDTHUSERID,$phone,$sender_number,$birthday_wishes,1);
                        }
                        //$this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$phone,$sender_number,$birthday_wishes);
                        sleep(2);
                        $someone_users = $this->User->find('first', array('conditions' => array('User.id' => $user_id)));
            ...
            if ($group['Group']['notify_signup'] == 1) {
                ...
                if ($SID == ''){
                    $this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$mobile,$sender_number,$message,1);
                }else{
                    $this->Bandwidth->sendsms($SID,$AUTHTOKEN,$BANDWIDTHUSERID,$mobile,$sender_number,$message,1);
                }
				//$this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$mobile,$sender_number,$message);
                $someone_users = $this->User->find('first', array('conditions' => array('User.id' => $user_id)));
                if (!empty($someone_users)) {
            ...
            if ($someone['User']['sms_balance'] > 0) {
                ...
                if (!empty($contactgroupid)) {
                    ...
                    if($double_optin==0){
                        ...
                        if ($SID == ''){
                            $this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$phone,$sender_number,$double_optin_msg,1);
                        }else{
                            $this->Bandwidth->sendsms($SID,$AUTHTOKEN,$BANDWIDTHUSERID,$phone,$sender_number,$double_optin_msg,1);
                        }
						//$this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$phone,$sender_number,$double_optin_msg);
                        //sleep(2);
                        $someone_users = $this->User->find('first', array('conditions' => array('User.id' => $user_id)));
            ...
            if ($contestkeywords['Contest']['winning_phone_number'] !=''){
                ...
                if ($SID == ''){
                    $this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$_REQUEST['From'],$_REQUEST['To'],$message1,1);
                }else{
                    $this->Bandwidth->sendsms($SID,$AUTHTOKEN,$BANDWIDTHUSERID,$_REQUEST['From'],$_REQUEST['To'],$message1,1);
                }
                //$this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$_REQUEST['From'],$_REQUEST['To'],$message1);
                $this->smsmail($user_id);
                exit;
            }
            ...
            $this->User->save($this->request->data);
            if ($SID == ''){
                $this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$_REQUEST['From'],$_REQUEST['To'],$contestkeywords['Contest']['system_message'],1);
            }else{
                $this->Bandwidth->sendsms($SID,$AUTHTOKEN,$BANDWIDTHUSERID,$_REQUEST['From'],$_REQUEST['To'],$contestkeywords['Contest']['system_message'],1);
            }
			//$this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$_REQUEST['From'],$_REQUEST['To'],$contestkeywords['Contest']['system_message']);
            $this->smsmail($user_id);
            //*********** Save to activity timeline
            ...
            if ($someone['User']['sms_balance'] > 0) {
                ...
                if ($this->User->id != '') {
                    ...
                    $length = mb_strlen($_REQUEST['Body'],"UTF-8");
			        $gsm = $this->is_gsm($_REQUEST['Body']);
			        if(trim($_REQUEST['eventType'])=="mms") {
			            $credits = 1;
			            if($SID ==''){
                            $response = $this->Bandwidth->messagedetails(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$_REQUEST['messageId']);
                        }else{
                            $response = $this->Bandwidth->messagedetails($SID,$AUTHTOKEN,$BANDWIDTHUSERID,$_REQUEST['messageId']);
                        }
                        
			            $ch = curl_init();
            		    curl_setopt($ch, CURLOPT_URL,$response->media[1]);
            		    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            		    if($SID ==''){
            		        curl_setopt($ch, CURLOPT_USERPWD, BANDWIDTH_KEY . ":" . BANDWIDTH_TOKEN);
            		    }else{
            		        curl_setopt($ch, CURLOPT_USERPWD, $SID . ":" . $AUTHTOKEN);
            		    }
            		    curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json'));
            		    $my_save_dir = "mms/";
                        $filename = "incomingmms".time().".jpg";
                        ...
                        curl_close($ch);
                        fclose($fp);
                        $this->request->data['Log']['image_url'] = SITE_URL .'/'. $complete_save_loc;
            
			        }elseif(!$gsm){//if (strlen($_REQUEST['Body']) != strlen(utf8_decode($_REQUEST['Body']))) {
                        $credits = ceil($length / 70);
                    } else {
                        $credits = ceil($length / 160);
                    }
                ...
                if ($contact_id > 0) {
                    ...
                    if ($someone['User']['sms_balance'] > 0 && $someone['User']['incoming_nonkeyword'] == 1) {
                        ...
                        if ($SID == ''){
                            ...
                            $response = $this->Bandwidth->sendsms($SID,$AUTHTOKEN,$BANDWIDTHUSERID,$to,$from,$message);
                        }
                        //$response =  $this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$to,$from,$message);
						$message_id = '';
						$status = '';
	...
	function Immediatelyresponder($user_id = null, $group_id = null, $to = null, $from = null){
        ...
        $BANDWIDTHUSERID = trim($users['User']['bandwidthuserid']);
        ...
        if ($assigned_number != '') {
            if (!empty($response)) {
                ...
                if ($users['User']['sms_balance'] > 0) {
                    ...
                    if ($sms_type == 1) {
                        if ($SID == ''){
                            $this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$to,$assigned_number,$body,1);
                        }else{
                            $this->Bandwidth->sendsms($SID,$AUTHTOKEN,$BANDWIDTHUSERID,$to,$assigned_number,$body,1);
                        }
						//$this->Bandwidth->sendsms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$to, $assigned_number, $body);
                        $credits = 1;
                    } else if ($sms_type == 2) {
                        $message_arr = explode(',', $image_url);
                        if ($SID == ''){
                            $this->Bandwidth->sendmms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$to,$assigned_number,$body,$message_arr,1);
                        }else{
                            $this->Bandwidth->sendmms($SID,$AUTHTOKEN,$BANDWIDTHUSERID,$to,$assigned_number,$body,$message_arr,1);
                        }
						//$this->Bandwidth->sendmms(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$to, $assigned_number, $body,$message_arr);
						$credits = 2;
                    }
	...
	function status(){
		...
		if(!empty($response)){
			...
			if ($tag == 0){
			    $someone = $this->Log->find('first', array('conditions' => array('Log.phone_number' => str_replace('+','',$to)), 'fields' => array('Log.id', 'Log.user_id', 'Log.text_message', 'Log.group_sms_id'), 'order' => array('Log.id' => 'DESC')));
			}
			$length = mb_strlen($text,"UTF-8");
			$gsm = $this->is_gsm($text);
			if (trim($eventType) == "sms") {
			    if(!$gsm) { 
				    $credits = ceil($length / 70);
			    } else {
				    $credits = ceil($length / 160);
			...
			if (!empty($someone)){
				if($eventType =='sms'){
					$log_arr['Log']['id'] = $someone['Log']['id'];
					$log_arr['Log']['sms_id'] = $messageId;
				    $log_arr['Log']['sms_status'] = $deliveryState;
				}else{
					$log_arr['Log']['id'] = $someone['Log']['id'];
					$log_arr['Log']['sms_id'] = $messageId;
				...
				if($deliveryCode > 0){
					$log_arr['Log']['error_message'] = $deliveryDescription;
				}
				...
				if (trim($deliveryState) == 'not-delivered') {
		...
		ob_end_clean();
		$file = fopen("debug/bandwidthsms".time().".txt", "w");
		fwrite($file, $out1);
		fclose($file);*/
	}
	...
	function voice(){
        ...
        $_REQUEST['To'] = str_replace('+', '', $_REQUEST['to']);
        $_REQUEST['From'] = str_replace('+', '', $_REQUEST['from']);
        $_REQUEST['CallSid'] = $_REQUEST['callId'];
        

       	if(trim($_REQUEST['eventType']=='answer')){
            ...
            if ($someone['User']['voice_balance'] > 0 && $active == 1) {
                if ($someone['User']['incomingcall_forward'] == 0 && $someone['User']['assign_callforward'] == trim($_REQUEST['To'])) {
                    ...
                    echo '<?xml version="1.0" encoding="UTF-8"?>';
                    echo '<Response>';
                    echo '<Transfer transferCallerId="+'.$_REQUEST['To'].'" transferTo="+'.$someone['User']['callforward_number'].'"></Transfer>';
                    echo '</Response>';
                } else {
                    $this->User->id = $someone['User']['id'];
                    ...
                    if ($someone['User']['welcome_msg_type'] == 1) {
                        $msg = "<PlayAudio>" . SITE_URL . '/mp3/' . $someone['User']['mp3'] . "</PlayAudio>";
                    } else {
                        $msg = "<SpeakSentence locale='en_US' voice='susan'>" . $someone['User']['defaultgreeting'] . "</SpeakSentence><PlayAudio volume='-4'>" . SITE_URL . "/audio/voicemailbeep.mp3</PlayAudio>";
                    }
                    header("content-type: text/xml");
                    echo "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n";
                    echo "<Response>";
                    echo $msg;
    				echo "<Record requestUrl='" . SITE_URL . "/bandwidths/voicerecording/" . $someone['User']['id'] . "' requestUrlTimeout = '30000' maxDuration='120' terminatingDigits='*' silenceTimeout='10'/>";
    				//echo "<SpeakSentence locale='en_US' voice='susan'>I did not receive a recording</SpeakSentence>
    				echo "</Response>";
                    exit;
                }
            }
		}elseif(trim($_REQUEST['eventType']=='transferComplete')){
            ...
            if($SID ==''){
                $response = $this->Bandwidth->calldetails(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$_REQUEST['callId']);
            }else{
                $response = $this->Bandwidth->calldetails($SID,$AUTHTOKEN,$BANDWIDTHUSERID,$_REQUEST['callId']);
            }
            date_default_timezone_set($someone['User']['timezone']);
            ...
            $this->Log->create();
            $this->request->data['Log']['user_id'] = $someone['User']['id'];
            $this->request->data['Log']['phone_number'] = str_replace('+', '', $response->displayName);
            $this->request->data['Log']['route'] = 'inbox';
            $this->request->data['Log']['msg_type'] = 'callforward';
            
            $date = new DateTime( $response->startTime );
            $date2 = new DateTime( $response->endTime );
            $diffInSeconds = $date2->getTimestamp() - $date->getTimestamp();

            $this->request->data['Log']['call_duration'] = gmdate("H:i:s", $diffInSeconds);
            
            if (isset($response->chargeableDuration)) {
               $minutes = ceil($response->chargeableDuration / 60);
            }
	...
	function voicerecording($user_id){
        $this->autoRender = false;
		...
		$_REQUEST['To'] = str_replace('+', '', $response->to);
        $_REQUEST['From'] = str_replace('+', '', $response->from);
        $_REQUEST['CallSid'] = $_REQUEST['callId'];
        Controller::loadModel('Log');
        $this->Log->recursive = -1;
        $someoneLog = $this->Log->find('first', array('conditions' => array('voice_url' => '' . trim($_REQUEST['CallSid']) . '')));
        if (isset($someoneLog['Log']['id']) && isset($_REQUEST['recordingUri'])) {
            $this->Log->id = $someoneLog['Log']['id'];
            if($SID ==''){
                $response = $this->Bandwidth->recordingdetails(BANDWIDTH_KEY,BANDWIDTH_TOKEN,BANDWIDTH_USER_ID,$_REQUEST['callId']);
            }else{
                $response = $this->Bandwidth->recordingdetails($SID,$AUTHTOKEN,$BANDWIDTHUSERID,$_REQUEST['callId']);
            }
            
            $ch = curl_init();
		    curl_setopt($ch, CURLOPT_URL,$response[0]->media);
		    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
		    if($SID ==''){
		        curl_setopt($ch, CURLOPT_USERPWD, BANDWIDTH_KEY . ":" . BANDWIDTH_TOKEN);
		    }else{
		    ...
		    curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json'));
		    $my_save_dir = "voicemail/";
            $filename = "voicemail".time().".wav";
            $complete_save_loc = $my_save_dir . $filename;
            $fp = fopen($complete_save_loc, 'wb');
            curl_setopt($ch, CURLOPT_FILE, $fp);
            curl_setopt($ch, CURLOPT_HEADER, 0);
            ...
            //$this->Log->saveField('voice_url', $response[0]->media);
            $this->Log->saveField('voice_url', SITE_URL .'/'. $complete_save_loc);
            $this->Log->saveField('sms_status','complete');
        }
        if (isset($_REQUEST['recordingUri'])) {
            Controller::loadModel('User');
            $someone = $this->User->find('first', array('conditions' => array('assigned_number' => '' . trim($_REQUEST['To']) . '')));
            if (empty($someone)) {
                app::import('Model', 'UserNumber');
	...
	function peoplecallrecordscript($group_id = null, $language = null, $pause = null, $forward = null, $forward_number = null){
        $this->autoRender = false;
		...
		if(trim($_REQUEST['eventType']=='answer')){
            ...
            if ($language = "en_US"){
                $voice = "susan";
            }elseif ($language = "en_UK"){
                $voice = "bridget";
            }elseif ($language = "es_mx"){
                $voice = "esperanza";
            }
    		if ($message_type == 1) {  // audio url
    			...
    			echo "<PlayAudio>" . SITE_URL . '/voice/' . $VoiceMessage['VoiceMessage']['audio'] . "</PlayAudio>";
    			echo "<Gather requestUrl='" . SITE_URL . "/bandwidths/do_not_call/" . $group_id . "/" . $forward . "/" . $forward_number . "' requestUrlTimeout='10000' maxDigits='1' terminatingDigits='*'>";
    			if ($forward == 1) {
    				echo "<SpeakSentence locale='" . $language . "' voice='" . $voice ."'>Please press 1 to speak with a representative or press 2 to be added to the do not call list.</SpeakSentence>";
    			} else {
    				echo "<SpeakSentence locale='" . $language . "' voice='" . $voice ."'>Please press 2 to be added to the do not call list.</SpeakSentence>";
    			}
    			echo "</Gather>";
                echo "</Response>";
                exit;
    			...
    			echo "<Gather requestUrl='" . SITE_URL . "/bandwidths/do_not_call/" . $group_id . "/" . $forward . "/" . $forward_number . "' requestUrlTimeout='10000' maxDigits='1' terminatingDigits='*'>";
    			echo "<Pause length='" . $pause . "'/>";
    			echo "<SpeakSentence locale='" . $language . "' voice='" . $voice ."'>" . $VoiceMessage['VoiceMessage']['text_message'] . "</SpeakSentence>";
    			if ($forward == 1) {
    				echo "<SpeakSentence locale='" . $language . "' voice='" . $voice ."'>Please press 1 to speak with a representative or press 2 to be added to the do not call list.</SpeakSentence>";
    			} else {
    				echo "<SpeakSentence locale='" . $language . "' voice='" . $voice ."'> Please press 2 to be added to the do not call list.</SpeakSentence>";
            ...
            $this->Log->create();
            $this->request->data['Log']['user_id'] = $someone['User']['id'];
            $this->request->data['Log']['phone_number'] = str_replace('+', '', $_REQUEST['To']);
            $this->request->data['Log']['route'] = 'inbox';
            $this->request->data['Log']['msg_type'] = 'callforward';
		...
		}elseif(trim($_REQUEST['eventType']=='hangup')){
            ...
            Controller::loadModel('Log');
            $this->Log->recursive = -1;
			$log = $this->Log->find('first', array('conditions' => array('Log.phone_number' => $_REQUEST['To'], 'Log.route' => 'inbox', 'Log.msg_type' => 'broadcast'), 'fields' => array('Log.id'), 'order' => array('Log.id' => 'DESC')));
			
            $this->request->data['Log']['id'] = $log['Log']['id'];
            $this->request->data['Log']['sms_status'] = $response->state;
            $this->request->data['Log']['voice_url'] = $_REQUEST['callId'];
            $this->request->data['Log']['route'] = 'inbox';
            $this->request->data['Log']['msg_type'] = 'broadcast';
        ...
        if ($_REQUEST['digits'] == 2) {
            ...
            if (!empty($Subscriber)) {
                ...
                if (!empty($Subscribergroup)) {
                    ...
                    echo "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n";
                    echo "<Response>
						<SpeakSentence locale='en_US' voice='susan'>You have successfully been un-subscribed and added to the do not call list</SpeakSentence>
					</Response>";
                }
            } 
            ...
            echo '<?xml version="1.0" encoding="UTF-8"?>';
			echo '<Response>';
			echo '<Transfer transferTo="+'.$forward_number.'"></Transfer>';
			echo '</Response>';
        } 
    }
