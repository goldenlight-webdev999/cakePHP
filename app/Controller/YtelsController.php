...
class YtelsController extends AppController{
	var $uses = 'Users';
    var $components = array('Ytel');
	...
	function searchbynumber(){
		...
		if(!empty($this->request->data)){
			...
			$response = $this->Ytel->listNumbers(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$areacode,$type);
			if($response->Message360->ResponseStatus == 0){
				...
				$this->set('response', $response->Message360->Phones->Phone);	
			}
	...
	function assignthisnumber(){
		...
		if(!empty($this->request->data['number'])){
			...
			$buyresponse = $this->Ytel->assignthisnumber(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$this->request->data['number']);
			
			if($buyresponse->Message360->SuccessCount > 0){
			    $response = $this->Ytel->updatethisnumber(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$this->request->data['number']);
				Controller::loadModel('User');
				$user_id = $this->Session->read('User.id');
				$someone = $this->User->find('first', array('conditions' => array('User.id' => $user_id)));
				if ($someone['User']['assigned_number'] == 0) {
					...
					$this->User->saveField('phone_sid', $buyresponse->Message360->BuyNumber->PhoneSid);
					$this->User->saveField('voice', 1);
					$this->User->saveField('sms', 1);										
					//if(isset($buyresponse->Message360->BuyNumber->MmsEnabled) && $buyresponse->Message360->BuyNumber->MmsEnabled){												
					...
					$arr['UserNumber']['phone_sid'] =  $buyresponse->Message360->BuyNumber->PhoneSid;
	...
	function sendsms($id = null){
        ...
        if ($userDetails['User']['sms_balance'] > 0) {
            $to = ($this->request->data['Ytel']['phone_number']) ? $this->request->data['Ytel']['phone_number'] : $this->request->data['Ytel']['phone'];
			...
			$body = $this->request->data['Ytel']['message'];  
	...
	function sms(){
		...
		ob_end_clean();
		$file = fopen("debug/ytel_incoming".time().".txt", "w");
		fwrite($file, $out1);
		fclose($file);*/
        ...
        $_REQUEST['media'] = trim($_REQUEST['MediaURL']);;
        $phone = $_REQUEST['From'];
                ...
                if (!empty($groupbroadcast)) {
                    ...
                    if (!empty($contactlist)) {
                        ...
                        if ($sms_balance < ($totalSubscriber * $contactcredits)) {
                            ...
                            if ($SID == ''){
                                $this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$_REQUEST['From'],$_REQUEST['To'],$message);
                            }else{
                                $this->Ytel->sendsms($SID,$AUTHTOKEN,$_REQUEST['From'],$_REQUEST['To'],$message);
                            }
                        ...
                        foreach ($contactlist as $contactlists) {
                            ...
                            $bodymsg = $checkmsgpart[1];
                            if ($SID == ''){
                                $response = $this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$tonumber,$fromnumber,$bodymsg);
                            }else{
                                $response = $this->Ytel->sendsms($SID,$AUTHTOKEN,$tonumber,$fromnumber,$bodymsg);
                            }
    								...
    								if (strpos($response_phone, ',') !== false) {
    									...
    									if(!empty($response_phone_arr)){
    										foreach($response_phone_arr as $response_phone_arrs){
    											try {
    												if ($SID == ''){
    													$this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$response_phone_arrs,$_REQUEST['To'],$response_text_msg);
    												}else{
    													$this->Ytel->sendsms($SID,$AUTHTOKEN,$response_phone_arrs,$_REQUEST['To'],$response_text_msg);
    												}
    									...
    									try {
    										if ($SID == ''){
    											$this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$response_phone,$_REQUEST['To'],$response_text_msg);
    										}else{
    											$this->Ytel->sendsms($SID,$AUTHTOKEN,$response_phone,$_REQUEST['To'],$response_text_msg);
    										}
    										$length = mb_strlen($response_text_msg,"UTF-8");
    										$gsm = $this->is_gsm($response_text_msg);
    										if(!$gsm){
        ...
        if (strtoupper(trim($confirmkeyword)) == strtoupper($_REQUEST['Text'])) {
            ...
            if (!empty($appointment)) {
                ...
                if ($SID == ''){
                    $this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$_REQUEST['From'],$_REQUEST['To'],$confirmmessage);
                }else{
                    $this->Ytel->sendsms($SID,$AUTHTOKEN,$_REQUEST['From'],$_REQUEST['To'],$confirmmessage);
                }
                $this->User->id = $someone['User']['id'];
                if ($this->User->id != '') {
                    $this->User->saveField('sms_balance', ($someone['User']['sms_balance'] - CHARGEINCOMINGSMS - 1));
            ...
            if (!empty($appointment)) {
                ...
                if ($SID == ''){
                    $this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$_REQUEST['From'],$_REQUEST['To'],$cancelmessage);
                }else{
                    $this->Ytel->sendsms($SID,$AUTHTOKEN,$_REQUEST['From'],$_REQUEST['To'],$cancelmessage);
                }
                $this->User->id = $someone['User']['id'];
                if ($this->User->id != '') {
                    $this->User->saveField('sms_balance', ($someone['User']['sms_balance'] - CHARGEINCOMINGSMS - 1));
            ...
            if (!empty($appointment)) {
                ...
                if ($SID == ''){
                    $this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$_REQUEST['From'],$_REQUEST['To'],$reschedulemessage);
                }else{
                    $this->Ytel->sendsms($SID,$AUTHTOKEN,$_REQUEST['From'],$_REQUEST['To'],$reschedulemessage);
                }
                $this->User->id = $someone['User']['id'];
                if ($this->User->id != '') {
                    $this->User->saveField('sms_balance', ($someone['User']['sms_balance'] - CHARGEINCOMINGSMS - 1));
            ...
            if ($someone['User']['sms_balance'] > 0) {
                ...
                if (!empty($contactgroupid)) {
                    ...
                    if (!empty($loyaltyuser)) {
                        ...
                        if ($SID == ''){
                            $this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$_REQUEST['From'],$_REQUEST['To'],$statusmsg);
                        }else{
                            $this->Ytel->sendsms($SID,$AUTHTOKEN,$_REQUEST['From'],$_REQUEST['To'],$statusmsg);
                        }
                                ...
                                if ($smsloyalty_arr['Smsloyalty']['reachgoal'] == 1) {
                                    ...
                                    if ($this->SmsloyaltyUser->save($loyalty_user)) {
                                        if ($smsloyalty_arr['Smsloyalty']['type'] == 1) {
                                            ...
                                            $sms = $msg . ' ' . $redeem;
                                            if ($SID == ''){
                                                $this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$_REQUEST['From'],$_REQUEST['To'],$sms);
                                            }else{
                                                $this->Ytel->sendsms($SID,$AUTHTOKEN,$_REQUEST['From'],$_REQUEST['To'],$sms);
                                            }
                                            //$this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$_REQUEST['From'],$_REQUEST['To'],$sms);
                                            $this->smsmail($someone['User']['id']);
                                            //*********** Save to activity timeline
											...
											if ($SID == ''){
                                                $this->Ytel->sendmms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$_REQUEST['From'],$_REQUEST['To'],$sms,$media);
                                            }else{
                                                $this->Ytel->sendmms($SID,$AUTHTOKEN,$_REQUEST['From'],$_REQUEST['To'],$sms,$media);
                                            }
											//$this->Ytel->sendmms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$_REQUEST['From'],$_REQUEST['To'],$sms,$media);
                                            $this->smsmail($someone['User']['id']);
                                            //*********** Save to activity timeline
                                    ...
                                    if ($SID == ''){
                                        $this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$_REQUEST['From'],$_REQUEST['To'],$msg);
                                    }else{
                                        $this->Ytel->sendsms($SID,$AUTHTOKEN,$_REQUEST['From'],$_REQUEST['To'],$msg);
                                    }
                                    //$this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$_REQUEST['From'],$_REQUEST['To'],$msg);
                                    $this->smsmail($someone['User']['id']);
                                    //*********** Save to activity timeline
        ...
        //****DOUBLE OPT-IN CHANGES****//    
            ...
            if ($sms_type == 1) {
                if ($SID == ''){
                    $this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$phone,$sender_number,$message);
                }else{
                    $this->Ytel->sendsms($SID,$AUTHTOKEN,$phone,$sender_number,$message);
                }
				//$this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$phone,$sender_number,$message);
				sleep(2);
                ...
                if ($SID == ''){
                    $this->Ytel->sendmms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$phone,$sender_number,$message,$message_arr);
                }else{
                    $this->Ytel->sendmms($SID,$AUTHTOKEN,$phone,$sender_number,$message,$message_arr);
                }
				//$this->Ytel->sendmms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$phone,$sender_number,$message,$message_arr);
				sleep(2);
                $this->Immediatelyresponder($user_id, $group_id, $phone, $sender_number);
            ...
            if ($contact_doubleoptin['ContactGroup']['subscribed_by_sms']==1){
                if ($someone['User']['capture_email_name'] == 0) {
                    ...
                    if ($SID == ''){
                        $this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$phone,$sender_number,$capture_email_name);
                    }else{
                        $this->Ytel->sendsms($SID,$AUTHTOKEN,$phone,$sender_number,$capture_email_name);
                    }
					//$this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$phone,$sender_number,$capture_email_name);
                    sleep(2);
                ...
                if ($group['Group']['bithday_enable'] == 1) {
                    if ($someone['User']['birthday_wishes'] == 0) {
                        ...
                        if ($SID == ''){
                            $this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$phone,$sender_number,$birthday_wishes);
                        }else{
                            $this->Ytel->sendsms($SID,$AUTHTOKEN,$phone,$sender_number,$birthday_wishes);
                        }
                        //$this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$phone,$sender_number,$birthday_wishes);
                        sleep(2);
                        $someone_users = $this->User->find('first', array('conditions' => array('User.id' => $user_id)));
            ...
            if ($group['Group']['notify_signup'] == 1) {
                ...
                if ($SID == ''){
                    $this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$mobile,$sender_number,$message);
                }else{
                    $this->Ytel->sendsms($SID,$AUTHTOKEN,$mobile,$sender_number,$message);
                }
				//$this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$mobile,$sender_number,$message);
                $someone_users = $this->User->find('first', array('conditions' => array('User.id' => $user_id)));
                if (!empty($someone_users)) {
            ...
            if ($someone['User']['sms_balance'] > 0) {
                if (empty($contact)) {
                    if (NUMVERIFY != '') {
                        ...
                        if($carrier->Message360->ResponseStatus == 1){
                            $this->request->data['Contact']['carrier'] = $carrier->Message360->Carrier->Network;
                ...
                if (!empty($contactgroupid)) {
                    ...
                    if (!empty($contact) && NUMVERIFY != '') {
                        ...
                        if($carrier->Message360->ResponseStatus == 1){
                            $contactarr['carrier'] = $carrier->Message360->Carrier->Network;
                    ...
                    if($double_optin==0){
                        ...
                        if ($SID == ''){
                            $this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$phone,$sender_number,$double_optin_msg);
                        }else{
                            $this->Ytel->sendsms($SID,$AUTHTOKEN,$phone,$sender_number,$double_optin_msg);
                        }
						//$this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$phone,$sender_number,$double_optin_msg);
                        //sleep(2);
                        $someone_users = $this->User->find('first', array('conditions' => array('User.id' => $user_id)));
            ...
            if ($contestkeywords['Contest']['winning_phone_number'] !=''){
                ...
                if ($SID == ''){
                    $this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$_REQUEST['From'],$_REQUEST['To'],$message1);
                }else{
                    $this->Ytel->sendsms($SID,$AUTHTOKEN,$_REQUEST['From'],$_REQUEST['To'],$message1);
                }
                //$this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$_REQUEST['From'],$_REQUEST['To'],$message1);
                $this->smsmail($user_id);
                echo "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n";
            ...
            $this->User->save($this->request->data);
            if ($SID == ''){
                $this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$_REQUEST['From'],$_REQUEST['To'],$contestkeywords['Contest']['system_message']);
            }else{
                $this->Ytel->sendsms($SID,$AUTHTOKEN,$_REQUEST['From'],$_REQUEST['To'],$contestkeywords['Contest']['system_message']);
            }
			//$this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$_REQUEST['From'],$_REQUEST['To'],$contestkeywords['Contest']['system_message']);
            $this->smsmail($user_id);
            //*********** Save to activity timeline
                ...
                if ($contact_id > 0) {
                    ...
                    if ($someone['User']['sms_balance'] > 0 && $someone['User']['incoming_nonkeyword'] == 1) {
                        ...
                        if ($SID == ''){
                            ...
                            $response = $this->Ytel->sendsms($SID,$AUTHTOKEN,$to,$from,$message);
                        }
                        //$response =  $this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$to,$from,$message);
						$message_id = '';
						$status = '';
        ...
        if ($assigned_number != '') {
            if (!empty($response)) {
                ...
                if ($users['User']['sms_balance'] > 0) {
                    ...
                    if ($sms_type == 1) {
                        if ($SID == ''){
                            $this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$to,$assigned_number,$body);
                        }else{
                            $this->Ytel->sendsms($SID,$AUTHTOKEN,$to,$assigned_number,$body);
                        }
						//$this->Ytel->sendsms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$to, $assigned_number, $body);
                        $credits = 1;
                    } else if ($sms_type == 2) {
                        $message_arr = explode(',', $image_url);
                        if ($SID == ''){
                            $this->Ytel->sendmms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$to,$assigned_number,$body,$message_arr);
                        }else{
                            $this->Ytel->sendmms($SID,$AUTHTOKEN,$to,$assigned_number,$body,$message_arr);
                        }
						//$this->Ytel->sendmms(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$to, $assigned_number, $body,$message_arr);
						$credits = 2;
                    }
	...
	function status($group_sms_id=null,$group_id=null,$userid=null){
		...
		$someone = $this->Log->find('first', array('conditions' => array('Log.sms_id' => $_REQUEST['MessageSid']), 'fields' => array('Log.id', 'Log.user_id', 'Log.text_message', 'Log.group_sms_id')));
		$length = mb_strlen($_REQUEST['Text'],"UTF-8");
		$gsm = $this->is_gsm($_REQUEST['Text']);
		
		if (!empty($someone)){
            $logid = $someone['Log']['id'];
            $userid = $someone['Log']['user_id'];
            $group_sms_id = $someone['Log']['group_sms_id'];
            $this->request->data['Log']['id'] = $logid;
        }else{
            $this->request->data['Log']['group_sms_id'] = $group_sms_id;
            $this->request->data['Log']['sms_id'] = $_REQUEST['MessageSid'];
            ...
            $this->request->data['Log']['sendfrom'] = str_replace('+', '', $_REQUEST['From']);
            if (isset($_REQUEST['MediaURL'])) {
                $this->request->data['Log']['image_url'] = $_REQUEST['MediaURL'];
                $this->request->data['Log']['text_message'] = $_REQUEST['Text'];
            } else {
                $this->request->data['Log']['text_message'] = $_REQUEST['Text'];
            }
            $this->request->data['Log']['msg_type'] = 'text';
            $this->request->data['Log']['route'] = 'outbox';
        }
            
        app::import('Model', 'GroupSmsBlast');
        $this->GroupSmsBlast = new GroupSmsBlast();
        $GroupSmsBlast = $this->GroupSmsBlast->find('first', array('conditions' => array('GroupSmsBlast.id' => $group_sms_id)));

        app::import('Model', 'User');
        $this->User = new User();
        $usersms = $this->User->find('first', array('conditions' => array('User.id' => $userid), 'fields' => array('User.timezone', 'User.sid', 'User.authtoken')));
        
        $SID = trim($usersms['User']['sid']);
        $AUTHTOKEN = trim($usersms['User']['authtoken']);
        
        $timezone = $usersms['User']['timezone'];
        date_default_timezone_set($timezone);
        
        if(trim($_REQUEST['MessageSid']) !=''){

            if ($SID == ''){
		        $response = $this->Ytel->messagedetails(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$_REQUEST['MessageSid']);
		    }else{
		        $response = $this->Ytel->messagedetails($SID,$AUTHTOKEN,$_REQUEST['MessageSid']);
		    }
            
            $Status = $response->Message360->Message->Status;
            
            if (strtoupper(trim($Status)) == 'UNDELIVERED' || strtoupper(trim($Status)) == 'FAILED') {
		...
		$out1 = ob_get_contents();
		ob_end_clean();
		$file = fopen("debug/ytelstatus".time().".txt", "w");
		fwrite($file, $out1);
		fclose($file);*/
	...
	function voice()
    {
		...
		$out1 = ob_get_contents();
		ob_end_clean();
		$file = fopen("debug/ytelvoice".time().".txt", "w");
		fwrite($file, $out1);
		fclose($file);*/
        ...
        if ($someone['User']['voice_balance'] > 0 && $active == 1) {
            if ($someone['User']['incomingcall_forward'] == 0 && $someone['User']['assign_callforward'] == trim($_REQUEST['To'])) {
                ...
                echo '<?xml version="1.0" encoding="UTF-8"?>';
                echo '<Response>';
                echo '<Dial action="' . SITE_URL . '/ytels/voicerecord">' . substr($someone['User']['callforward_number'],1) . '</Dial>';
                //echo '<Dial action="' . SITE_URL . '/ytels/voicerecord">8157149179 </Dial>';
                echo '</Response>';
                ...
                echo $msg;
                echo "<Record
							action='" . SITE_URL . "/ytels/voicerecording'
							method='POST'
							maxLength='60'
							finishOnKey='*'
							playBeep='true'
    ...
    function voicerecord()
    {
        ...
        if(strtoupper(trim($_REQUEST['DialCallStatus'])) == "COMPLETED"){
            
            $response = $this->Ytel->calldetails(YTEL_ACCOUNTSID,YTEL_AUTH_TOKEN,$_REQUEST['DialCallSid']);
            $durationjson = json_decode($response->Message360->Call->RequestInspector[0]->ResponseBody);

            $date = new DateTime( $durationjson->StartTime );
            $date2 = new DateTime( $durationjson->EndTime );
            $duration = $date2->getTimestamp() - $date->getTimestamp();
            ...
            if($response->Message360->ResponseStatus > 0){
                ...
                if (isset($_REQUEST['RecordUrl'])) {
                    $url_to_image = $_REQUEST['RecordUrl'];
                ...
                //if (isset($_REQUEST['DialCallDuration'])) {
                    $minute = gmdate("H:i:s", $durationjson->DialCallBilledDuration);
    ...
    function voicerecording()
    {
        ...
        if (isset($someoneLog['Log']['id']) && isset($_REQUEST['RecordUrl'])) {
	...
	function peoplecallrecordscript($group_id = null, $loop = null, $language = null, $pause = null, $forward = null, $forward_number = null)
    {
        ...
        if ($message_type == 1) {  // audio url
            ...
            echo "<Gather action='" . SITE_URL . "/ytels/do_not_call/" . $group_id . "/" . $forward . "/" . $forward_number . "' timeout='3' numDigits='1' finishOnKey='*'>";
            if ($forward == 1) {
                echo "<Say voice ='" . $language . "'>Please press 1 to speak with a representative or press 2 to be added to the do not call list.</Say>";
            } else {
                echo "<Say voice ='" . $language . "'>Please press 2 to be added to the do not call list.</Say>";
            }
            echo "</Gather>";
            echo "</Response>";
            exit;
            ...
            echo "<Response>";
            echo "<Pause length='" . $pause . "'/>";
            echo "<Say voice='" . $language . "' loop='" . $loop . "'>" . $msg . "</Say>";
            echo "<Gather action='" . SITE_URL . "/ytels/do_not_call/" . $group_id . "/" . $forward . "/" . $forward_number . "' timeout='3' numDigits='1' finishOnKey='*'>";
            if ($forward == 1) {
                echo "<Say voice ='" . $language . "'>Please press 1 to speak with a representative or press 2 to be added to the do not call list.</Say>";
            } else {
	...
	function do_not_call($group_id = null, $forward = null, $forward_number = null)
    {
        ...
        if ($_REQUEST['Digits'] == 2) {
            ...
            echo '<?xml version="1.0" encoding="UTF-8"?>';
            echo '<Response>';
            echo '<Dial>' . substr($forward_number,1) . '</Dial>';
            //echo '<Dial action="' . SITE_URL . '/ytels/voicerecord/' . $forward . '">' . substr($forward_number,1) . '</Dial>';
            //echo '<Dial>8153027443</Dial>';
            echo '</Response>';
            exit;
        } else {
            header("content-type: text/xml");
    ...
    function sendcallStatus($lastinsertID = null,$forward = null)
    {
            ...
            if (strtoupper(trim($_REQUEST['Status'])) == 'COMPLETED') {
                ...
                if (!empty($someone)) {
                    ...
                    if (!empty($usersms)) {
                        ...
                        $minute = gmdate("H:i:s", $_REQUEST['BilledDuration']);
                        $minute_arr = explode(':', $minute);
                        $hour = 0;
                        $mint = 0;
                        $secnd = 0;
