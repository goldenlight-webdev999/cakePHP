...
class SignalwiresController extends AppController{
	var $uses = 'Users';
    var $components = array('Signalwire');
		...
		if(!empty($this->request->data)){
			...
			if($this->request->data['type']=='Local'){
				/*if(!empty($this->request->data['state'])){
					$fields['InRegion'] = $this->request->data['state'];
				}*/
				/*if(!empty($this->request->data['zip'])){
					$fields['InPostalCode'] = $this->request->data['zip'];
				}*/
				if(!empty($this->request->data['areaCode'])){
					$fields['AreaCode'] = $this->request->data['areaCode'];
				}
				$response = $this->Signalwire->listNumbers(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$type,$country,$fields);
				if(!isset($response)){
					$this->Session->setFlash(__("There were no numbers that matched the search criteria. Try again.", true));	
				}else{
					$this->set('response', $response->available_phone_numbers);	
				}
			}else{
			    if(!empty($this->request->data['areaCode'])){
					$fields['AreaCode'] = $this->request->data['areaCode'];
	...
	function assignthisnumber(){
		...
		if(!empty($this->request->data['number'])){
			...
			$response = $this->Signalwire->assignthisnumber(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$this->request->data['number']);
			
			if(isset($response->account_sid)){
				Controller::loadModel('User');
				$user_id = $this->Session->read('User.id');
				$someone = $this->User->find('first', array('conditions' => array('User.id' => $user_id)));
				if ($someone['User']['assigned_number'] == 0) {
					...
					$arr['UserNumber']['phone_sid'] =  $response->sid;
	...
	function sendsms($id = null){
        ...
        if ($userDetails['User']['sms_balance'] > 0) {
            $to = ($this->request->data['Signalwire']['phone_number']) ? $this->request->data['Signalwire']['phone_number'] : $this->request->data['Signalwire']['phone'];
			...
			$body = $this->request->data['Signalwire']['message'];  
	...
	function sms(){
		...
		ob_end_clean();
		$file = fopen("debug/signalwireincoming".time().".txt", "w");
		fwrite($file, $out1);
		fclose($file);*/
        ...
        $_REQUEST['media'] = trim($_REQUEST['MediaUrl0']);;
        $phone = $_REQUEST['From'];
                ...
                if (!empty($groupbroadcast)) {
                    ...
                    if (!empty($contactlist)) {
                        ...
                        if ($sms_balance < ($totalSubscriber * $contactcredits)) {
                            ...
                            if ($SID == ''){
                                $this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$_REQUEST['From'],$_REQUEST['To'],$message);
                            }else{
                                $this->Signalwire->sendsms($SID,$AUTHTOKEN,$SIGNALWIRESPACE,$_REQUEST['From'],$_REQUEST['To'],$message);
                            }
                        ...
                        foreach ($contactlist as $contactlists) {
                            ...
                            $bodymsg = $checkmsgpart[1];
                            if ($SID == ''){
                                $response = $this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$tonumber,$fromnumber,$bodymsg);
                            }else{
                                $response = $this->Signalwire->sendsms($SID,$AUTHTOKEN,$SIGNALWIRESPACE,$tonumber,$fromnumber,$bodymsg);
                            }
    								...
    								if (strpos($response_phone, ',') !== false) {
    									...
    									if(!empty($response_phone_arr)){
    										foreach($response_phone_arr as $response_phone_arrs){
    											try {
    												if ($SID == ''){
    													$this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$response_phone_arrs,$_REQUEST['To'],$response_text_msg);
    												}else{
    													$this->Signalwire->sendsms($SID,$AUTHTOKEN,$SIGNALWIRESPACE,$response_phone_arrs,$_REQUEST['To'],$response_text_msg);
    												}
    									...
    									try {
    										if ($SID == ''){
    											$this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$response_phone,$_REQUEST['To'],$response_text_msg);
    										}else{
    											$this->Signalwire->sendsms($SID,$AUTHTOKEN,$SIGNALWIRESPACE,$response_phone,$_REQUEST['To'],$response_text_msg);
    										}
    										$length = mb_strlen($response_text_msg,"UTF-8");
    										$gsm = $this->is_gsm($response_text_msg);
    										if(!$gsm){
        ...
        if (strtoupper(trim($confirmkeyword)) == strtoupper($_REQUEST['Body'])) {
            ...
            if (!empty($appointment)) {
                ...
                if ($SID == ''){
                    $this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$_REQUEST['From'],$_REQUEST['To'],$confirmmessage);
                }else{
                    $this->Signalwire->sendsms($SID,$AUTHTOKEN,$SIGNALWIRESPACE,$_REQUEST['From'],$_REQUEST['To'],$confirmmessage);
                }
                $this->User->id = $someone['User']['id'];
                if ($this->User->id != '') {
                    $this->User->saveField('sms_balance', ($someone['User']['sms_balance'] - CHARGEINCOMINGSMS - 1));
            ...
            if (!empty($appointment)) {
                ...
                if ($SID == ''){
                    $this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$_REQUEST['From'],$_REQUEST['To'],$cancelmessage);
                }else{
                    $this->Signalwire->sendsms($SID,$AUTHTOKEN,$SIGNALWIRESPACE,$_REQUEST['From'],$_REQUEST['To'],$cancelmessage);
                }
                $this->User->id = $someone['User']['id'];
                if ($this->User->id != '') {
                    $this->User->saveField('sms_balance', ($someone['User']['sms_balance'] - CHARGEINCOMINGSMS - 1));
            ...
            if (!empty($appointment)) {
                ...
                if ($SID == ''){
                    $this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$_REQUEST['From'],$_REQUEST['To'],$reschedulemessage);
                }else{
                    $this->Signalwire->sendsms($SID,$AUTHTOKEN,$SIGNALWIRESPACE,$_REQUEST['From'],$_REQUEST['To'],$reschedulemessage);
                }
                $this->User->id = $someone['User']['id'];
                if ($this->User->id != '') {
                    $this->User->saveField('sms_balance', ($someone['User']['sms_balance'] - CHARGEINCOMINGSMS - 1));
            ...
            if ($someone['User']['sms_balance'] > 0) {
                ...
                if (!empty($contactgroupid)) {
                    ...
                    if (!empty($loyaltyuser)) {
                        ...
                        if ($SID == ''){
                            $this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$_REQUEST['From'],$_REQUEST['To'],$statusmsg);
                        }else{
                            $this->Signalwire->sendsms($SID,$AUTHTOKEN,$SIGNALWIRESPACE,$_REQUEST['From'],$_REQUEST['To'],$statusmsg);
                        }
                                ...
                                if ($smsloyalty_arr['Smsloyalty']['reachgoal'] == 1) {
                                    ...
                                    if ($this->SmsloyaltyUser->save($loyalty_user)) {
                                        if ($smsloyalty_arr['Smsloyalty']['type'] == 1) {
                                            ...
                                            $sms = $msg . ' ' . $redeem;
                                            if ($SID == ''){
                                                $this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$_REQUEST['From'],$_REQUEST['To'],$sms);
                                            }else{
                                                $this->Signalwire->sendsms($SID,$AUTHTOKEN,$SIGNALWIRESPACE,$_REQUEST['From'],$_REQUEST['To'],$sms);
                                            }
                                            //$this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$_REQUEST['From'],$_REQUEST['To'],$sms);
                                            $this->smsmail($someone['User']['id']);
                                            //*********** Save to activity timeline
											...
											if ($SID == ''){
                                                $this->Signalwire->sendmms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$_REQUEST['From'],$_REQUEST['To'],$sms,$media);
                                            }else{
                                                $this->Signalwire->sendmms($SID,$AUTHTOKEN,$SIGNALWIRESPACE,$_REQUEST['From'],$_REQUEST['To'],$sms,$media);
                                            }
											//$this->Signalwire->sendmms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$_REQUEST['From'],$_REQUEST['To'],$sms,$media);
                                            $this->smsmail($someone['User']['id']);
                                            //*********** Save to activity timeline
                                    ...
                                    if ($SID == ''){
                                        $this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$_REQUEST['From'],$_REQUEST['To'],$msg);
                                    }else{
                                        $this->Signalwire->sendsms($SID,$AUTHTOKEN,$SIGNALWIRESPACE,$_REQUEST['From'],$_REQUEST['To'],$msg);
                                    }
                                    //$this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$_REQUEST['From'],$_REQUEST['To'],$msg);
                                    $this->smsmail($someone['User']['id']);
                                    //*********** Save to activity timeline
        ...
        //****DOUBLE OPT-IN CHANGES****//    
            ...
            if ($sms_type == 1) {
                if ($SID == ''){
                    $this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$phone,$sender_number,$message);
                }else{
                    $this->Signalwire->sendsms($SID,$AUTHTOKEN,$SIGNALWIRESPACE,$phone,$sender_number,$message);
                }
				//$this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$phone,$sender_number,$message);
				sleep(2);
                ...
                if ($SID == ''){
                    $this->Signalwire->sendmms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$phone,$sender_number,$message,$message_arr);
                }else{
                    $this->Signalwire->sendmms($SID,$AUTHTOKEN,$SIGNALWIRESPACE,$phone,$sender_number,$message,$message_arr);
                }
				//$this->Signalwire->sendmms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$phone,$sender_number,$message,$message_arr);
				sleep(2);
                $this->Immediatelyresponder($user_id, $group_id, $phone, $sender_number);
            ...
            if ($contact_doubleoptin['ContactGroup']['subscribed_by_sms']==1){
                if ($someone['User']['capture_email_name'] == 0) {
                    ...
                    if ($SID == ''){
                        $this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$phone,$sender_number,$capture_email_name);
                    }else{
                        $this->Signalwire->sendsms($SID,$AUTHTOKEN,$SIGNALWIRESPACE,$phone,$sender_number,$capture_email_name);
                    }
					//$this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$phone,$sender_number,$capture_email_name);
                    sleep(2);
                ...
                if ($group['Group']['bithday_enable'] == 1) {
                    if ($someone['User']['birthday_wishes'] == 0) {
                        ...
                        if ($SID == ''){
                            $this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$phone,$sender_number,$birthday_wishes);
                        }else{
                            $this->Signalwire->sendsms($SID,$AUTHTOKEN,$SIGNALWIRESPACE,$phone,$sender_number,$birthday_wishes);
                        }
                        //$this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$phone,$sender_number,$birthday_wishes);
                        sleep(2);
                        $someone_users = $this->User->find('first', array('conditions' => array('User.id' => $user_id)));
            ...
            if ($group['Group']['notify_signup'] == 1) {
                ...
                if ($SID == ''){
                    $this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$mobile,$sender_number,$message);
                }else{
                    $this->Signalwire->sendsms($SID,$AUTHTOKEN,$SIGNALWIRESPACE,$mobile,$sender_number,$message);
                }
				//$this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$mobile,$sender_number,$message);
                $someone_users = $this->User->find('first', array('conditions' => array('User.id' => $user_id)));
                if (!empty($someone_users)) {
            ...
            if ($someone['User']['sms_balance'] > 0) {
                ...
                if (!empty($contactgroupid)) {
                    ...
                    if($double_optin==0){
                        ...
                        if ($SID == ''){
                            $this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$phone,$sender_number,$double_optin_msg);
                        }else{
                            $this->Signalwire->sendsms($SID,$AUTHTOKEN,$SIGNALWIRESPACE,$phone,$sender_number,$double_optin_msg);
                        }
						//$this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$phone,$sender_number,$double_optin_msg);
                        //sleep(2);
                        $someone_users = $this->User->find('first', array('conditions' => array('User.id' => $user_id)));
            ...
            if ($contestkeywords['Contest']['winning_phone_number'] !=''){
                ...
                if ($SID == ''){
                    $this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$_REQUEST['From'],$_REQUEST['To'],$message1);
                }else{
                    $this->Signalwire->sendsms($SID,$AUTHTOKEN,$SIGNALWIRESPACE,$_REQUEST['From'],$_REQUEST['To'],$message1);
                }
                //$this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$_REQUEST['From'],$_REQUEST['To'],$message1);
                $this->smsmail($user_id);
                echo "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n";
            ...
            $this->User->save($this->request->data);
            if ($SID == ''){
                $this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$_REQUEST['From'],$_REQUEST['To'],$contestkeywords['Contest']['system_message']);
            }else{
                $this->Signalwire->sendsms($SID,$AUTHTOKEN,$SIGNALWIRESPACE,$_REQUEST['From'],$_REQUEST['To'],$contestkeywords['Contest']['system_message']);
            }
			//$this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$_REQUEST['From'],$_REQUEST['To'],$contestkeywords['Contest']['system_message']);
            $this->smsmail($user_id);
            //*********** Save to activity timeline
                ...
                if ($contact_id > 0) {
                    ...
                    if ($someone['User']['sms_balance'] > 0 && $someone['User']['incoming_nonkeyword'] == 1) {
                        ...
                        if ($SID == ''){
                            ...
                            $response = $this->Signalwire->sendsms($SID,$AUTHTOKEN,$SIGNALWIRESPACE,$to,$from,$message);
                        }
                        //$response =  $this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$to,$from,$message);
						$message_id = '';
						$status = '';
	...
	function Immediatelyresponder($user_id = null, $group_id = null, $to = null, $from = null){
        ...
        $SIGNALWIRESPACE = trim($users['User']['signalwirespace']);
        ...
        if ($assigned_number != '') {
            if (!empty($response)) {
                ...
                if ($users['User']['sms_balance'] > 0) {
                    ...
                    if ($sms_type == 1) {
                        if ($SID == ''){
                            $this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$to,$assigned_number,$body);
                        }else{
                            $this->Signalwire->sendsms($SID,$AUTHTOKEN,$SIGNALWIRESPACE,$to,$assigned_number,$body);
                        }
						//$this->Signalwire->sendsms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$to, $assigned_number, $body);
                        $credits = 1;
                    } else if ($sms_type == 2) {
                        $message_arr = explode(',', $image_url);
                        if ($SID == ''){
                            $this->Signalwire->sendmms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$to,$assigned_number,$body,$message_arr);
                        }else{
                            $this->Signalwire->sendmms($SID,$AUTHTOKEN,$SIGNALWIRESPACE,$to,$assigned_number,$body,$message_arr);
                        }
						//$this->Signalwire->sendmms(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$to, $assigned_number, $body,$message_arr);
						$credits = 2;
                    }
	...
	function status(){
		...
		if (!empty($someone)){
            ...
            if (trim($Status) == 'undelivered' || trim($Status) == 'failed') {
                app::import('Model', 'User');
                $this->User = new User();
                $usersms = $this->User->find('first', array('conditions' => array('User.id' => $user_id), 'fields' => array('User.sms_balance', 'User.id', 'User.sid', 'User.authtoken', 'User.signalwirespace')));
                ...
                if ($SID == ''){
			        $response = $this->Signalwire->messagedetails(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$_REQUEST['SmsSid']);
			    }else{
			        $response = $this->Signalwire->messagedetails($SID,$AUTHTOKEN,$SIGNALWIRESPACE,$_REQUEST['SmsSid']);
			    }
        		...
        		$out1 = ob_get_contents();
        		ob_end_clean();
        		$file = fopen("debug/signalwirestatus".time().".txt", "w");
        		fwrite($file, $out1);
        		fclose($file);*/
	...
	function voice()
    {
        ...
        if ($someone['User']['voice_balance'] > 0 && $active == 1) {
            if ($someone['User']['incomingcall_forward'] == 0 && $someone['User']['assign_callforward'] == trim($_REQUEST['To'])) {
                ...
                echo '<?xml version="1.0" encoding="UTF-8"?>';
                echo '<Response>';
                echo '<Dial action="' . SITE_URL . '/signalwires/voicerecord" record="do-not-record">' . $someone['User']['callforward_number'] . '</Dial>';
                echo '</Response>';
                ...
                echo $msg;
                echo "<Record
							action='" . SITE_URL . "/signalwires/voicerecording'
							method='POST'
							maxLength='60'
							finishOnKey='*'
							/>
    ...
    function voicerecord($forwardfrombroadcast = 0)
    {
        ...
        if (isset($_REQUEST['RecordingUrl'])) {
            ...
            $this->request->data['Log']['voice_url'] = $url_to_image;
        }
        $this->request->data['Log']['route'] = 'inbox';
	...
	function peoplecallrecordscript($group_id = null, $loop = null, $language = null, $pause = null, $forward = null, $forward_number = null)
    {
        ...
        if ($message_type == 1) {  // audio url
            ...
            echo "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n";
            echo "<Response>";
            echo "<Gather action='" . SITE_URL . "/signalwires/do_not_call/" . $group_id . "/" . $forward . "/" . $forward_number . "' timeout='3' numDigits='1' finishOnKey='*'>";
	...
	function do_not_call($group_id = null, $forward = null, $forward_number = null)
    {
        ...
        if ($_REQUEST['Digits'] == 2) {
            ...
            echo '<?xml version="1.0" encoding="UTF-8"?>';
            echo '<Response>';
            echo '<Dial action="' . SITE_URL . '/signalwires/voicerecord/' . $forward . '" record="do-not-record" answerOnBridge="true">' . $forward_number . '</Dial>';
            echo '</Response>';
            exit;
        } else {
            header("content-type: text/xml");
    ...
    function faxsend($sourcepage)
    {
        ...
        $AUTHTOKEN = trim($userDetails['User']['authtoken']);
        $SIGNALWIRESPACE = trim($userDetails['User']['signalwirespace']);
        ...
        if ($userDetails['User']['voice_balance'] > 0) {
            ...
            $to = $this->request->data['signalwires']['phone_number'];
            $quality = $this->request->data['Contact']['quality'];
        
            if($SID == ''){
                $response = $this->Signalwire->sendfax(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE, $to, $from, $filepath, $quality);
            }else{
                $response = $this->Signalwire->sendfax($SID, $AUTHTOKEN, $SIGNALWIRESPACE, $to, $from, $filepath, $quality);
            }
            ...
            if ($response->status == 400) {
                ...
                $this->request->data['Log']['sms_id'] = $response->sid;
            }
    ...
    function faxcallstatus()
    {
        ...
        if (!empty($someone)) {
            ...
            if (trim($Status) == 'failed'){
                $this->request->data['Log']['error_message'] = "Fax failed to send. Make sure you are sending to a proper fax-enabled number.";
            } elseif (trim($Status) == 'delivered') {
                app::import('Model', 'User');
                $this->User = new User();
    ...
    function fax()
    {
        ...
        if ($someone['User']['voice_balance'] > 0 && $someone['User']['active'] == 1) {
            echo "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n";
            echo "<Response>";
		    echo '<Receive action="' . SITE_URL . '/signalwires/faxreceived"></Receive>';
		    echo "</Response>";
        }else{
            echo "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n";
            echo "<Response>";
    ...
    function faxreceived()
    {
        ...
        $response = $this->faxget($_REQUEST['FaxSid'],$someone['User']['id']);
        $faxduration = $response->duration;
    ...
    function faxget($faxid,$user_id)
    {
        ...
        if($SID == ''){
            $response = $this->Signalwire->getfax(SIGNALWIRE_KEY,SIGNALWIRE_TOKEN,SIGNALWIRE_SPACE,$faxid); 
        }else{
            $response = $this->Signalwire->getfax($SID,$AUTHTOKEN,$SIGNALWIRESPACE,$faxid);
        }
