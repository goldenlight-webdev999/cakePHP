...
class NexmosController extends AppController
{
    ...
    function sms()
    {
        ...
        if(trim($someone['User']['sid']) !=''){
            $this->Nexmomessage->Key = trim($someone['User']['sid']);
        }else{
            $this->Nexmomessage->Key = NEXMO_KEY;
        }

        if(trim($someone['User']['authtoken']) !=''){
            $this->Nexmomessage->Secret = trim($someone['User']['authtoken']); 
        }else{
            $this->Nexmomessage->Secret = NEXMO_SECRET; 
        }
			...
			if(!empty($multicontestquestion)){
				...
				if(empty($multicontestsubscribers)){
					...
					if($this->MultiContestSubscriber->save($multicontest_subscriber)){
						...
						$this->Nexmomessage->sendsms($_REQUEST['From'], $_REQUEST['To'], $message);
						exit();
					}
				}else{
    								...
    								if (strpos($response_phone, ',') !== false) {
    									...
    									if(!empty($response_phone_arr)){
    										foreach($response_phone_arr as $response_phone_arrs){
    											try {
    												$this->Nexmomessage->sendsms($response_phone_arrs, $_REQUEST['To'], $response_text_msg);
    									...
    									try {
    										$this->Nexmomessage->sendsms($response_phone, $_REQUEST['To'], $response_text_msg);
    										$length = mb_strlen($response_text_msg,"UTF-8");
    										$gsm = $this->is_gsm($response_text_msg);
    										if(!$gsm){
    											$credits = ceil($length / 70) + CHARGEINCOMINGSMS;
...
//*********** Save to activity timeline
            ...
            if ($this->User->id != '') {
                ...
                if(!$gsm){//if (strlen($_REQUEST['text']) != strlen(utf8_decode($_REQUEST['text']))) {
                    $credits = ceil($length / 70);
                } else {
                    $credits = ceil($length / 160);
                }
        ...
        if (!empty($response)) {
            ...
            if ($users['User']['sms_balance'] > 0) {
                ...
                if(trim($users['User']['sid']) !=''){
                    $this->Nexmomessage->Key = trim($users['User']['sid']);
                }else{
                    $this->Nexmomessage->Key = NEXMO_KEY;
                }

                if(trim($users['User']['authtoken']) !=''){
                    $this->Nexmomessage->Secret = trim($users['User']['authtoken']); 
                }else{
                    $this->Nexmomessage->Secret = NEXMO_SECRET; 
                }
                $response = $this->Nexmomessage->sendsms($to, $from, $body);
