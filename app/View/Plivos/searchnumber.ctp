...
function assignthisnumber(numbertoassign){
	...
	data: {number:numbertoassign,sms:sms,mms:mms,voice:voice,country:$('#country_code').val()},
		success: function(result) {
			if(result=='sucess'){
         ...
         <div class="table-scrollable">
				...
				if(!empty($AvailablePhoneNumbers)){
					...
					<form method="POST">
						...
						<?php } ?>
						<?php if((isset($phone['mms_enabled'])) && ($phone['mms_enabled']==1)){ ?>
						
						(MMS)
						<input type="hidden" name="sms" id="mms" value="1">
						<?php }else{ ?>
						<input type="hidden" name="sms" id="mms" value="0">
						<?php } ?>
						</label>
