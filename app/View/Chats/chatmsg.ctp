...
<li class="media border-dotted" style="margin-bottom:10px;background:#ebf9ff">
					<?php
						...
						if(is_null($log['Log']['name']) ||trim($log['Log']['name'] =='')){
						   $name = $this->Session->read('User.first_name').' '.$this->Session->read('User.last_name');
						}else{
						   $name = $log['Log']['name']; 
						}
					?>
				...
				<!--<div class="main-chat">-->
					...
					<?php }else{ ?>
						...
						<h3><!--<?php echo ucfirst($this->Session->read('User.first_name').' '.$this->Session->read('User.last_name'))?>-->
						<?php echo $name ?>
						</h3>
					<?php } ?>
...
<?php if(trim($log['Log']['image_url'])!=''){ ?>
                                      <p style=" word-wrap: break-word;"><a href="<?php echo $log['Log']['image_url']; ?>"><img src="<?php echo $log['Log']['image_url']; ?>" width="100px" height="100px"></a></p>
                                <?php }else {?>
				      <p style=" word-wrap: break-word;"><?php echo ucfirst($log['Log']['text_message']); ?></p>
                                <?php } ?>
