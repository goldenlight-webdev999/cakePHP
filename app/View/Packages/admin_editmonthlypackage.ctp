...
<?php echo $this->Form->create('Package',array('style'=>'width:98%'));?>
	<fieldset>
		<legend><?php echo('Edit Monthly Package'); ?></legend>
	<?php
	$Option=array('1'=>'Active','0'=>'Inactive');
	$CarryOverOption=array('1'=>'Yes','0'=>'No');
...
'Zimbabwe'=>'Zimbabwe');
		...
		echo $this->Form->input('MonthlyPackage.text_messages_credit',array('label'=>'<strong>SMS Credits</strong>'));
		echo $this->Form->input('MonthlyPackage.voice_messages_credit',array('label'=>'<strong>Voice Credits</strong>'));
		echo $this->Form->input('MonthlyPackage.credits_carryover',array('type'=>'select','options'=>$CarryOverOption));
        ...
        echo '<select name="data[MonthlyPackage][username]">';
        echo '<option value=""></option>';
	    foreach($users as $user){ 
				echo '<option value="'.$user["username"].'"';
				if ($package['MonthlyPackage']['username'] == $user['username']) {
				    echo ' selected';
				}
				echo '>'.$user["username"];
				echo '</option>';
	    ...
	    echo '</select></div>';
		echo $this->Form->input('MonthlyPackage.status',array('type'=>'select','options'=>$Option));
