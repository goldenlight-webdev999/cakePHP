...
function selectcountry(){
  $.ajax({type: "POST",url:"<?php echo SITE_URL ?>/ytels/searchcountry",
...
function assignthisnumber(numbertoassign){
	$.ajax({
		type: "POST",
		url:"<?php echo SITE_URL ?>/ytels/assignthisnumber",
		data: {
			 ...
			 <div class="table-scrollable">
				<table class="table table-striped table-bordered table-hover">
					...
					<tbody>
						...
						?>
						<tr>
							<td><?php echo $phone_arr->PhoneNumber; ?></td>
							<!--<?php if($type == 'Local'){?>
								<td><?php echo $phone_arr->rate_center; ?></td>
								<td><?php echo $phone_arr->region; ?></td>
							<?php }?>-->
							<td><a  class="btn blue btn-outline btn-sm" href="#" onClick="assignthisnumber(<?php echo $phone_arr->PhoneNumber; ?>)" >Get this number</a></td>
						</tr>
						<?php }?>
					</tbody>
				</table>
